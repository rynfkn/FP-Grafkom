...
export function createPedestalTable({
  ...
  const baseStep = createBox(
    baseWidth,
    baseStepHeight,
    baseWidth,
    baseMaterial,
    0,
    basePlateHeight + baseStepHeight / 2,
    0
  );
  ...
  const lowerCol = createBox(
    colWidth,
    lowerColH,
    colWidth,
    baseMaterial,
    0,
    currentY + lowerColH / 2,
    0
  );
...
export function createBench() {
  ...
  positions.forEach((posX) => {
    const inlay = createBox(
      inlaySize,
      inlayThickness,
      inlaySize,
      darkWoodMat,
      posX,
      seatY + topThickness / 2 + 0.001,
      0
    );
  ...
  // Kiri Depan
  benchGroup.add(
    createBox(legSize, legH, legSize, lightWoodMat, -legX, legY, legZ)
  );
  // Kanan Depan
  benchGroup.add(
    createBox(legSize, legH, legSize, lightWoodMat, legX, legY, legZ)
