...
function createLamp() {
  ...
  const mount = new THREE.Mesh(
    new THREE.CylinderGeometry(0.06, 0.06, 0.02, 32),
    blackMat
  );
  ...
  const rim = new THREE.Mesh(
    new THREE.CylinderGeometry(bodyRadius - 0.02, bodyRadius - 0.02, 0.02, 32),
    chromeMat
  );
  ...
  const bulb = new THREE.Mesh(
    new THREE.CircleGeometry(bodyRadius - 0.04, 32),
    bulbMat
  );
...
function createCeiling() {
  ...
  const whiteCeilingMat = new THREE.MeshStandardMaterial({
    color: 0xfdf5e6,
    roughness: 0.9,
  ...
  xPositions.forEach((posX) => {
    ...
    const beam = new THREE.Mesh(
      new THREE.BoxGeometry(beamWidth, beamHeight, length),
      beamMat
    );
  ...
  zPositions.forEach((posZ) => {
    ...
    const beam = new THREE.Mesh(
      new THREE.BoxGeometry(length, beamHeight - 0.02, beamWidth),
      beamMat
    );
...
export function createWall(
  ...
  if (bottomHeight > 0.01) {
    const bottomWall = new THREE.Mesh(
      new THREE.BoxGeometry(width, bottomHeight, thickness),
      material
    );
  ...
  if (topHeight > 0.01) {
    const topWall = new THREE.Mesh(
      new THREE.BoxGeometry(width, topHeight, thickness),
      material
    );
  ...
  sortedWindows.forEach((win) => {
    ...
    if (gapWidth > 0.01) {
      const pillar = new THREE.Mesh(
        new THREE.BoxGeometry(gapWidth, windowAreaHeight, thickness),
        material
      );
      pillar.position.set(
        currentX + gapWidth / 2,
        bottomHeight + windowAreaHeight / 2,
        0
      );
  ...
  if (remainingWidth > 0.01) {
    const endPillar = new THREE.Mesh(
      new THREE.BoxGeometry(remainingWidth, windowAreaHeight, thickness),
      material
    );
...
export function createWindows(windowConfig, wallThickness = 0.4) {
  ...
  windowConfig.forEach((win) => {
    ...
    const top = new THREE.Mesh(
      new THREE.BoxGeometry(win.width, frameW, frameD),
      frameMat
    );
    ...
    const bottom = new THREE.Mesh(
      new THREE.BoxGeometry(win.width, frameW, sillDepth),
      frameMat
    );
    ...
    const left = new THREE.Mesh(
      new THREE.BoxGeometry(frameW, sideH, frameD),
      frameMat
    );
    ...
    // Horizontal
    const hMid = new THREE.Mesh(
      new THREE.BoxGeometry(win.width - frameW * 2, barThick, barDepth),
      frameMat
    );
    ...
    const vMid = new THREE.Mesh(
      new THREE.BoxGeometry(barThick, sideH, barDepth),
      frameMat
    );
    ...
    const glass = new THREE.Mesh(
      new THREE.BoxGeometry(win.width - frameW, win.height - frameW, 0.05),
      glassMat
    );
